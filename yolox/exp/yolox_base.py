    ...
    def preprocess(self, inputs, targets, tsize):
        ...
        if scale_x != 1 or scale_y != 1:
            image = nn.functional.interpolate(
                image, size=tsize, mode="bilinear", align_corners=False
            )
            event = nn.functional.interpolate(
                event, size=(int(h/2),int(w/2)), mode="bilinear", align_corners=False
            )
