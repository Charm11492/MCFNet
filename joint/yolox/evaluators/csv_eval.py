    ...
    with torch.no_grad():
        ...
        for index in range(len(dataset)):
            ...
            if torch.cuda.is_available():
                ...
                # print(torch.tensor(data[0]).permute(2, 0, 1).cuda().float().unsqueeze(dim=0).shape)
                output= retinanet(torch.tensor(data[0]).permute(2, 0, 1).cuda().float().unsqueeze(dim=0))
                ...
                for output_item in output:
                    ...
                    if output_item is not None:
                        ...
                        # Extract relevant information from each item in the output list
                        boxes = output_item[:, 0:4]
                        # preprocessing: resize
                        scale = 1.0
                        boxes /= scale

                        labels = output_item[:, 6]
                        scores = output_item[:, 4] * output_item[:, 5]

                        # Append the processed information to the respective lists
                        boxes_list.append(boxes)
                        labels_list.append(labels)
                        scores_list.append(scores)

                # Convert the lists to appropriate data structures if needed (e.g., torch.Tensor)
                boxes_tensor = torch.cat(boxes_list, dim=0) if boxes_list else torch.tensor([])
                labels_tensor = torch.cat(labels_list, dim=0) if labels_list else torch.tensor([])
                scores_tensor = torch.cat(scores_list, dim=0) if scores_list else torch.tensor([])
             
                #scores, labels, boxes = retinanet(torch.tensor(data['img']).permute(2, 0, 1).cuda().float().unsqueeze(dim=0))
            else:
                output = retinanet(data['img'].permute(2, 0, 1).float().unsqueeze(dim=0))
                output = output.cpu()
                ...
                labels = output[:, 6]
                scores = output[:, 4] * output[:, 5]
            scores_tensor = scores_tensor.cpu().numpy()
            labels_tensor = labels_tensor.cpu().numpy()
            boxes_tensor = boxes_tensor.cpu().numpy()

            # correct boxes for image scale
            print(f'boxes:{boxes_tensor.shape}')
            print(f'labels:{labels_tensor}')
            print(f'scores:{scores_tensor.shape}')
            # boxes /= scale
            # select indices which have a score above the threshold
            indices = np.where(scores_tensor > score_threshold)[1]
            print(indices)
            if indices.shape[0] > 0:
                # select those scores
                scores_tensor = scores_tensor[:,indices]
                print(scores)
            
                # find the order with which to sort the scores
                scores_sort = np.argsort(-scores_tensor)[:max_detections]
                scores_sort=scores_sort.flatten()
                print(scores_sort)

                # select detections
                image_boxes      = boxes_tensor[:,:,indices[scores_sort]]
                image_scores     = scores_tensor[:,scores_sort]
                image_labels     = labels_tensor[:,indices[scores_sort]]
                print(f'boxes:{image_boxes.shape}')
                print(f'labels:{image_labels}')
                print(f'scores:{image_scores.shape}')
                image_detections = np.concatenate([image_boxes, np.expand_dims(image_scores, axis=0), np.expand_dims(image_labels, axis=0)], axis=1)
