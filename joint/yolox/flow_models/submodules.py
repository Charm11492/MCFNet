...
class EccentricDownsample(nn.Module):
    def __init__(self, input_size, output_size, kernel_size=3, stride=2, padding=1, norm=None):
        super(EccentricDownsample, self).__init__()
        self.kernel_size = kernel_size
        self.stride = stride
        self.padding = padding
        self.norm = norm
        self.offsets = [-1, 0, 1]  # 偏移量
        self.convs = nn.ModuleList([
            ConvLayer(input_size, output_size, kernel_size, stride, padding, norm) for _ in range(5)
        ])
        
        self.final_conv = nn.Conv2d(5 * output_size, output_size, kernel_size=1)
    ...
    def forward(self, x):
        ...
        out = [
            self.convs[0](x_left),
            self.convs[1](x_right),
            self.convs[2](x_up),
            self.convs[3](x_down),
            self.convs[4](x_center),
        ]
