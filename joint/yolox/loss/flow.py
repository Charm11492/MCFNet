...
class EventWarping(nn.Module):
    ...
    def __init__(self,loss_scaling=True):
    ...
    def forward(self, flow_list, event_list, pol_mask):
        ...
        for flow in flow_list:
            ...
            # )
             # accumulate count of nonzero pixels
            # num_nonzero_pixels += torch.sum(bw_iwe_pos > 0).item() + torch.sum(bw_iwe_neg > 0).item()
            num_nonzero_pixels += torch.sum((bw_iwe_pos > 0) | (bw_iwe_neg > 0)).item()
            ...
            if self.loss_scaling:
                ...
                fw_loss /= torch.sum(fw_nonzero_px, dim=1)+1e-9
                # a= torch.tensor(153600.0)
                # fw_loss /= a
                ...
                bw_loss /= torch.sum(bw_nonzero_px, dim=1)+1e-9
                # bw_loss /= a
            ...
            #     + self.weight * (flow_dx.sum() + flow_dy.sum())
            # )
            loss += (fw_loss + bw_loss + self.weight * (flow_dx.sum() + flow_dy.sum())/flow_dx.shape[1])

            # scale the loss by dividing by the number of nonzero pixels
...
class AveragedIWE(nn.Module):
    ...
    def forward(self, flow, event_list, pol_mask):
        ...
        fw_iwe_pos_contrib = torch.zeros((flow.shape[0], self.res[0] * self.res[1], 1))
        fw_iwe_neg_contrib = torch.zeros((flow.shape[0], self.res[0] * self.res[1], 1))
        for b in range(0, self.batch_size):
            ...
            b_fw_iwe_pos_contrib = torch.zeros((self.res[0] * self.res[1], 1))
            ...
            b_fw_iwe_neg_contrib = torch.zeros((self.res[0] * self.res[1], 1))
