...
def visualize_assign(img, boxes, coords, match_results, save_name=None) -> np.ndarray:
    ...
    """
    print(img)
    img = np.sum(img.transpose((2, 0, 1)), 0)

    img = (img / np.max(img)) * 255
    left_event_b = img * 127 / abs(img).max() + 127
    # print(left_event_b)
    ...
    for box_id, box in enumerate(boxes):
        ...
        if assign_coords.numel() == 0:
            ...
            color = (0, 0, 255)
            cv2.putText(
                left_event_3c, "unmatched", (int(x1), int(y1) - 5),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 1
            )
        else:
            for coord in assign_coords:
                # draw assigned anchor
                cv2.circle(left_event_3c, (int(coord[0]), int(coord[1])), 3, color, -1)
        cv2.rectangle(left_event_3c, (int(x1), int(y1)), (int(x2), int(y2)), color, 2)

    if save_name is not None:
        cv2.imwrite(save_name, left_event_3c)

    return left_event_3c
