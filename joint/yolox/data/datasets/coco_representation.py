...
class COCODataset(CacheDataset):
    ...
    def load_image(self, index):
        ...
        # event_file = os.path.join(self.data_dir, file_name[0], 'h5', file_name[-1].replace('.npz','.npy'))
        event_file = os.path.join(self.event_dir, file_name[0], file_name[-1].replace('.npz','.npy'))
        ...
        # event_file = os.path.join(self.data_dir, file_name[0], 'h5', file_name[-1].replace('.npz','.png'))
        # print(event_file)
        # img_s = cv2.imread(event_file, cv2.IMREAD_COLOR)

        image = np.load(event_file).astype('float32')
        ...
        event[:, :, 0] = (image[:, :, 0] + image[:, :, 1]) / 2  # 第一个通道
        event[:, :, 1] = (image[:, :, 2] + image[:, :, 3]) / 2  # 第二个通道
        event[:, :, 2] = image[:, :, 4]  # 第三个通道不变

        # cv2.imwrite(os.path.join(save_path,os.path.splitext(img_name)[0] +'.png'),event)

        # print(event_file)
        # image = cv2.imread(event_file)
        # print(event.shape)
